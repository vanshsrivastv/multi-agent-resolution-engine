...
`evaluation/dataset.json` holds 60 hand-written tickets, each with an expected category and expected final status (not just category, since a ticket can be correctly classified and still fail to resolve correctly). `evaluation/run_evaluation.py` runs the real pipeline (real Groq, Qdrant, Razorpay, no mocks) against it.
...
**1. Unit tests + a dedicated routing-precision check.** `tests/` (72 tests, fully mocked, no network) covers every agent, the retry logic, and graph routing in isolation. On top of that, `evaluation/billing_routing_precision.py` runs 50 hand-written, unambiguously billing-shaped messages through the real Triage Agent (real Groq) and checks how many actually clear the 90% confidence gate into the Billing Agent. **Result: 50/50 (100%).**

**2. LLM-as-a-judge for hallucination.** Category/status accuracy says nothing about whether a *resolved* reply actually stayed grounded in its source document. `evaluation/judge_hallucination.py` has a second, independent Groq call grade every resolved technical reply against the exact document it cited.

First run: **5/8 (62%) grounded**, a real, unflattering number. Two of the three flags traced back to the same root cause: the knowledge base had no dedicated "how to enable 2FA" doc, only one about troubleshooting rejected codes, so RAG matched the wrong document and the reply then added an unstated "a team member will follow up." The third was the model generalizing a cause the doc ties to one specific error code onto unrelated cases. All three are fixable, not just explainable. Added the missing `enabling_two_factor_auth.md` doc, tightened the system prompt to forbid unstated promises and cross-case generalization, and lowered the reply's sampling temperature to 0.2 to reduce embellishment. Re-ran the same judge afterward: **7/8 (88%) grounded**. The one remaining flag is a genuine, documented knowledge-base coverage gap (a generic "login fails, no error code" doc doesn't exist, only one specific to error 500) rather than the model inventing something, left as-is rather than chased further, to avoid overfitting the prompt to these exact 8 test cases. Full reasoning per case in `evaluation/judge_results.json`.
